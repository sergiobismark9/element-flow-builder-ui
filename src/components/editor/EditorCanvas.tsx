...
export const EditorCanvas = ({
  ...
  const handleComponentDrop = (e: React.DragEvent, targetIndex: number) => {
    ...
    const componentType = e.dataTransfer.getData("text/plain");
    if (componentType) {
      onAddComponent(componentType, undefined, undefined, targetIndex);
    }
  };
  ...
  const renderComponents = (items: Component[], depth = 0) => {
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50">
      <div className="min-h-full p-6">
        ...
        >
          {components.length === 0 ? (
            <DragDropZone onAddComponent={(type) => onAddComponent(type)} />
          ) : (
